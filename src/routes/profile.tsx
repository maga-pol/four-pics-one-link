    ...
    <RacingShell>
      ...
      <section className="grid gap-4 lg:grid-cols-[0.85fr_1.15fr]">
        <div className="border border-[#303030] bg-[#181818] p-6">
          <div className="flex items-center gap-4">
            {user.user_metadata?.avatar_url ? (
              <img src={user.user_metadata.avatar_url} alt="" className="h-20 w-20 border border-[#303030]" />
            ) : (
          ...
          <div className="mt-6 border border-[#5a4218] bg-[#2a1f08] p-4">
            <div className="text-[11px] font-bold uppercase tracking-[0.12em] text-[#f5c518]">Rank</div>
        ...
        <div className="grid grid-cols-2 gap-3 md:grid-cols-3">
          <PremiumStat icon={<Coins className="h-5 w-5" />} label="Coins" value={game.coins ?? 250} />
          <PremiumStat icon={<Trophy className="h-5 w-5" />} label="Wins" value={game.wins ?? 0} />
          <PremiumStat icon={<Flag className="h-5 w-5" />} label="Total races" value={game.totalRaces ?? 0} />
          <PremiumStat icon={<Gauge className="h-5 w-5" />} label="Quizzes" value={game.totalQuizzesCompleted ?? 0} />
          <PremiumStat icon={<Flame className="h-5 w-5" />} label="Win streak" value={game.winStreak ?? 0} />
        ...
        <div className="grid gap-4 md:grid-cols-2">
          <div className="border border-[#303030] bg-[#181818] p-5">
            <div className="mb-3 flex items-center gap-2 text-[#f5c518]">
              ...
              <span className="text-[11px] font-bold uppercase tracking-[0.12em]">Favorite car</span>
            </div>
            <CarFigure car={selectedCar} compact />
            <div className="mt-3 font-display text-xl uppercase text-white">{selectedCar.name}</div>
            <div className="text-[11px] font-bold uppercase tracking-[0.12em] text-[#969696]">{selectedCar.team}</div>
          </div>
          <div className="border border-[#303030] bg-[#181818] p-5">
            <div className="mb-3 flex items-center gap-2 text-[#f5c518]">
              <UserCircle className="h-5 w-5" />
              <span className="text-[11px] font-bold uppercase tracking-[0.12em]">Favorite driver</span>
            </div>
            <DriverFigure driver={selectedDriver} size="small" />
            <div className="mt-3 font-display text-xl uppercase text-white">{selectedDriver.name}</div>
